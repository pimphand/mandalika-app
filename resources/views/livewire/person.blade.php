        ...
        <div class="card user-info-card mb-3">
            <div class="card-body d-flex align-items-center">
                <div class="user-profile me-3">
                    <img loading="lazy" src="{{ auth()->user()->photo }}" alt="" style="max-height: 100px;">
                </div>
                <div class="user-info">
                ...
                <div class="card-body">
                    <div class="direction-rtl mb-3">
                        ...
                        @foreach ($profile['data']['status'] as $order)
                            <a style="pointer-events: none; cursor: not-allowed;"
                                class="btn btn-{{ $status[$order['status']] }} m-1"
                                href="#">{{ $statusId[$order['status']] }}<span
                                    class="ms-1 badge bg-{{ $status[$order['status']] }}">{{ $order['total'] }}</span></a>
                            @php
                        ...
                        <div class="skill-data">
                            ...
                            <div class="skill-name d-flex align-items-center justify-content-between">
                                <p class="mb-1">Target Penjualan :
                                    Rp. {{ number_format($profile['data']['user']['target_sales'] ?? 0) }}</p>
                                <small class="mb-1"><span>Sedang Berjalan : Rp.
                                        {{ number_format($profile['data']['order']['total'] ?? 0) }}
                                </small>
                            </div>
                            @php
                                $target_sales = $profile['data']['user']['target_sales'] ?? 0;
                                $current_sales = $profile['data']['order']['total'] ?? 0;
                                $omzet_items = $profile['data']['order']['total_items'] ?? 0;
                                $target_items = $profile['data']['user']['omzet_items'] ?? 0;

                                // Cegah pembagian oleh nol
                                $percentage = $target_sales > 0 ? ($current_sales / $target_sales) * 100 : 0;
                                $formatted_percentage = number_format($percentage, 2);

                                $percentage2 = $target_items > 0 ? ($omzet_items / $target_items) * 100 : 0;
                                $formatted_percentage2 = number_format($percentage2, 2);
                            @endphp

                            <!-- Progress -->
                            <div class="progress" style="height: 15px;">
                                <div class="progress-bar" style="width: {{ $formatted_percentage }}%;color: #fff"
                                    role="progressbar" aria-valuenow="{{ $formatted_percentage }}" aria-valuemin="0"
                    ...
                    <div class="skill-progress-bar d-flex align-items-center">
                        ...
                        <div class="skill-data">
                            <!-- Skill Name -->
                            <div class="skill-name d-flex align-items-center justify-content-between">
                                <p class="mb-1">Target Item Terjual : {{ $profile['data']['user']['omzet_items'] }}
                                </p>
                                <small class="mb-1"><span>Item Terjual : {{ $omzet_items }}
                                </small>
                            </div>
                            <!-- Progress -->
                            <div class="progress" style="height: 15px;">
                                <div class="progress-bar bg-info" style="width: {{ $formatted_percentage }}%;"
                                    role="progressbar" aria-valuenow="{{ $formatted_percentage }}" aria-valuemin="0"
                                    aria-valuemax="100"> {{ $formatted_percentage2 }}%
                                </div>
                            </div>
                        </div>
                    </div>
